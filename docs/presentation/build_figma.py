#!/usr/bin/env python3
# Figma 마케팅 디자인 시스템: 흑백 에디토리얼 + 큰 파스텔 색블록. 별도 출력 파일.
import os
...
BLACK=C("000000"); WHITE=C("ffffff"); HAIR=C("e6e6e6"); SOFT=C("f7f7f5"); GRAY=C("555555")
LIME=C("dceeb1"); LILAC=C("c5b0f4"); CREAM=C("f4ecd6"); PINK=C("efd4d4"); MINT=C("c8e6cd"); CORAL=C("f3c9b6"); NAVY=C("1f1d3d")
MAGENTA=C("ff3d8b")
SANS="Apple SD Gothic Neo"; MONO="Menlo"
# 액터: 서로 다른 파스텔
A_CUST=LILAC; A_STAFF=MINT; A_SYS=CORAL
...
def panel(s,l,t,w,h,fill,rad=0.28,line=None):
...
def box(s,l,t,w,h,fill,runs,align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE,rad=0.18,line=None,wrap=True,shape=MSO_SHAPE.ROUNDED_RECTANGLE,spc=None):
...
def head(s,eyebrow,title,ink=BLACK,sub_color=GRAY):
    tb(s,0.85,0.6,11.6,0.5,[[(eyebrow,15,False,GRAY,MONO)]],spc=0.6)
    tb(s,0.82,1.05,11.7,1.3,[[(title,40,True,ink,SANS)]],anchor=MSO_ANCHOR.TOP,spc=-0.8)

def arrow(s,l,t,sz=30,col=BLACK):
...
# 1. 타이틀 (네이비 블록)
s=slide(NAVY)
tb(s,1.0,2.2,11.3,0.5,[[("FINAL PRESENTATION",16,False,C("c5b0f4"),MONO)]],spc=0.8)
tb(s,0.95,2.75,11.6,1.7,[[("설계대로 구현하고, AI를 통제하다",50,True,WHITE,SANS)]],spc=-1.0)
tb(s,1.0,4.55,11.3,0.8,[[("의료·자동차 보험 시스템",24,False,C("cdbff7"),SANS)]],spc=-0.3)
...
for (lab,col,role),x in zip(data,xs):
    box(s,x,2.5,2.5,2.5,col,[[(lab,30,True,BLACK,SANS)]],shape=MSO_SHAPE.OVAL,wrap=False)
    tb(s,x-1.0,5.25,4.5,0.7,[[(role,21,True,BLACK,SANS)]],align=PP_ALIGN.CENTER,spc=-0.3)

# 3. 두 가지 보험
s=slide(); head(s,"PRODUCTS","두 가지 보험")
for nm,ess,col,x in [("의료보험","아플 때 — 병원비",LIME,1.45),("자동차보험","사고 났을 때 — 수리·치료비",CORAL,7.0)]:
    panel(s,x,2.55,4.9,3.0,col,rad=0.16)
    tb(s,x,3.35,4.9,1.0,[[(nm,36,True,BLACK,SANS)]],align=PP_ALIGN.CENTER,spc=-0.6)
    tb(s,x,4.5,4.9,0.7,[[(ess,21,False,BLACK,SANS)]],align=PP_ALIGN.CENTER,spc=-0.3)

# 4. 의료보험 — 분기
s=slide(); head(s,"MEDICAL","병원비를 보험금으로 돌려준다")
box(s,1.0,3.9,3.0,1.3,LILAC,[[("병원비 청구",23,True,BLACK,SANS)]],wrap=False)
arrow(s,4.3,3.15,28); arrow(s,4.3,5.05,28)
box(s,5.2,2.7,7.2,1.35,MINT,[[("100만 원 미만  →  바로 입금",22,True,BLACK,SANS)]],align=PP_ALIGN.LEFT,wrap=False)
box(s,5.2,4.55,7.2,1.35,PINK,[[("100만 원 이상  →  직원 확인 후 입금",22,True,BLACK,SANS)]],align=PP_ALIGN.LEFT,wrap=False)

# 5. 자동차보험 — 흐름
s=slide(); head(s,"AUTO","사고 피해를 보상한다")
steps=[("사고 접수",CREAM),("직원이 맡음",CORAL),("보상액 사정",CORAL),("계좌 입금",MINT)]
iw,gap=2.55,0.6; total=len(steps)*iw+(len(steps)-1)*gap; x=(SW-total)/2
for i,(st,col) in enumerate(steps):
    box(s,x,3.7,iw,1.4,col,[[(st,22,True,BLACK,SANS)]],wrap=False)
...
def journey(eye,t,steps,vtext):
    ...
    for i,(who,lab,act) in enumerate(steps):
        box(s,x+iw/2-0.75,2.55,1.5,1.5,cmap[who],[[(lab,20,True,BLACK,SANS)]],shape=MSO_SHAPE.OVAL,wrap=False)
        box(s,x,4.3,iw,1.2,cmap[who],[[(act,22,True,BLACK,SANS)]],wrap=False)
        if i<n-1: arrow(s,x+iw+0.08,4.55)
        x+=iw+gap
    tb(s,0.85,5.95,11.6,0.9,[[(vtext,20,False,BLACK,SANS)]],align=PP_ALIGN.LEFT,spc=-0.3)
...
doms=[("사용자",LILAC),("상품",LIME),("계약",CREAM),("청구",CORAL),("심사",MINT),("사고이력",PINK)]
cw,gap=1.92,0.28; total=len(doms)*cw+(len(doms)-1)*gap; x=(SW-total)/2
for d,col in doms:
    box(s,x,3.3,cw,1.3,col,[[(d,23,True,BLACK,SANS)]],wrap=False); x+=cw+gap
...
for name,key,same,diff in comps:
    if name=="__CODE__":
        ...
        panel(s,0.95,2.5,11.4,2.0,SOFT,rad=0.06)
    ...
    tb(s,0.85,2.0,5.9,0.45,[[("설계",15,False,GRAY,MONO)]],align=PP_ALIGN.CENTER,spc=0.5)
    tb(s,6.6,2.0,5.9,0.45,[[("코드 역설계",15,False,GRAY,MONO)]],align=PP_ALIGN.CENTER,spc=0.5)
    img_fit(s,os.path.join(CL,f"{key}-design.png"),0.85,2.45,5.9,3.0)
    img_fit(s,os.path.join(CL,f"{key}-code.png"),6.6,2.45,5.9,3.0)
    box(s,0.85,5.65,5.9,1.3,MINT,[[("같은 점",16,True,BLACK,SANS)],[(same,15,False,BLACK,SANS)]],align=PP_ALIGN.LEFT,rad=0.12)
    box(s,6.6,5.65,5.9,1.3,PINK,[[("다른 점",16,True,BLACK,SANS)],[(diff,15,False,BLACK,SANS)]],align=PP_ALIGN.LEFT,rad=0.12)
...
panel(s,0.85,2.0,5.7,1.6,MINT,rad=0.14)
tb(s,1.2,2.2,5.1,1.3,[[("사람",22,True,BLACK,SANS)],[("설계·결정(ADR)·리뷰",18,False,BLACK,SANS)]],anchor=MSO_ANCHOR.MIDDLE)
panel(s,6.75,2.0,5.7,1.6,LILAC,rad=0.14)
...
for i,st in enumerate(steps):
    box(s,x,4.1,cw,1.1,CREAM if st!="TDD 구현" else CORAL,[[(st,20,True,BLACK,SANS)]],wrap=False)
...
cards=[("성급한 추상화","실익 없는 계층 분리 → 되돌림",LIME),("명세와 불일치","자동 배정 → 수동 배정 정정",CREAM),
       ("계층 패턴 일탈","점검으로 식별·관리",MINT),("용어·도메인 오염","용어집·ADR로 사전 차단",PINK)]
pos=[(0.85,2.1),(6.75,2.1),(0.85,4.0),(6.75,4.0)]
for (h,fix,col),(x,y) in zip(cards,pos):
    panel(s,x,y,5.7,1.7,col,rad=0.12)
    tb(s,x+0.4,y+0.25,5.0,1.3,[[(h,22,True,BLACK,SANS)],[(fix,18,False,BLACK,SANS)]],anchor=MSO_ANCHOR.MIDDLE,spc=-0.3)
tb(s,0.85,6.0,11.6,0.8,[[("AI는 빠르지만, 방향은 사람이 잡았다.",22,True,BLACK,SANS)]],align=PP_ALIGN.CENTER,spc=-0.3)

# 20. 결론 (네이비)
s=slide(NAVY)
tb(s,1.0,2.4,11.3,0.5,[[("CONCLUSION",16,False,C("c5b0f4"),MONO)]],spc=0.8)

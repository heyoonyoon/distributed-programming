...
### 제외 ❌ (이유)
- **맞춤 보험료 계산(`calculatePremium`)** — UC01은 "월 보험료 **예시**"만 요구하고,
  맞춤 견적의 입력(병력·차량정보)은 UC02 가입 폼에서 처음 수집된다. 자바 abstract는
  "껍데기만" 둘 수 없으므로(자식이 본문 강제) **메서드 자체를 Epic 1엔 만들지 않고**
  호출 흐름이 생기는 UC02 Epic에서 추가한다(ADR 0001).
...
| 주제 | 결정 | 근거 |
|------|------|------|
| `calculatePremium` 범위 | **예시 보험료만 노출, 메서드는 Epic 1에 만들지 않음** | UC01은 "예시"만 명시. 맞춤 입력은 UC02에서 수집. abstract "껍데기만"은 불가하므로 UC02 Epic에서 추가(ADR 0001). |
...
| 보험료 산출 기준(UC01 6단계) | **`description`에 녹임 (새 컬럼 없음)** | 다이어그램에 없는 필드 신설 금지. `premiumBasis` 별도 필드 두지 않음. (취조 반영) |
| 키워드 필터 중복 | **`SELECT DISTINCT p`** | 상품-보장항목 JOIN 시 한 상품이 여러 항목에 매칭돼도 1회만 반환. (취조 반영) |
| 동적 필터 구현 | **단일 JPQL + null-guard** | 선택 파라미터를 `:param IS NULL OR ...`로 무시. 필터 3개 규모엔 Specification 과함. (취조 반영) |
...
| `calculatePremium(holder)` | **Epic 1에서 만들지 않음**(ADR 0001). 자바 abstract는 "껍데기만" 불가 — 자식이 본문 강제됨. 호출 흐름이 생기는 UC02 Epic에서 추가. |
...
- **상세 응답** `ProductDetailResponse`: 목록 필드 + `description`,
  `coverageItems`: `CoverageItemResponse[]`(`itemName`, `coverageLimit`, `deductible`)
  - 보험료 산출 기준(UC01 6단계)은 **별도 필드 없이 `description`에 텍스트로 녹인다**
    (다이어그램에 없는 컬럼 신설 안 함).
- `type` 필수. `minPremium`/`maxPremium`/`keyword`는 선택. 필터는 **단일 JPQL + null-guard**
  (`:param IS NULL OR ...`)로 처리한다(Specification 안 씀 — 필터 3개 규모엔 과함).
...
2. `ProductService`가 필터 조건으로 `ProductRepository` 동적 조회
   ...
   - JOIN 시 한 상품이 여러 보장항목에 매칭되면 중복 행 발생 → **`SELECT DISTINCT p`** 로 상품 단위 1회만 반환
